        ...
        <div class="row card0">
            <div class="card1 col-lg-8 col-md-7"><small>the.weather</small>
                ...
                <div class="row px-3 mt-3 mb-3">
                    <h1 id="temperature" class="large-font mr-3">{{$celsius}}&#176;</h1>
                    <div class="d-flex flex-column mr-3">
                        <h2 id="nameCity" class="mt-3 mb-0">{{$nameCity}}</h2> <small id="timezone">{{$time}}</small>
                    </div>
                    <div class="d-flex flex-column text-center">
                        <h3 class="fa fa-sun-o mt-4"></h3> <small id="weather">{{$weather}}</small>
                    </div>
                </div>
            </div>
            <div class="card2 col-lg-4 col-md-5">
                <div class="row px-3"><input type="text" id="city" name="location" placeholder="Another location" class="mb-5">
                    <div id="check" class="fa fa-search mb-5 mr-0 text-center"></div>
                </div>
                <div class="mr-5">
...
{{--            <select class="form-control mb-3" id="changeCity" aria-label="Default select example">--}}
...
{{--                    <h2 id="nameCity">{{$nameCity}}</h2>--}}
{{--                    <h1 id="temperature">{{$celsius}}<sup>°  C </sup> </h1>--}}
{{--                    <h3 id="weather" class="my-0">Weather {{$weather}}</h3>--}}
{{--                    <h4 id="timezone" class="my-0 mt-2">{{$time}}</h4>--}}
{{--                </div>--}}
{{--                <div class="div2"> </div>--}}
{{--            </div>--}}
{{--        </div>--}}
    ...
    $(document).ready(function () {
        $('#city').on('keyup', function (e) {
            if(e.keyCode == 13 ){
                var city = $('#city').val();
                console.log(city);
                $.get("{{route('weather.index')}}" + "/" + city, function (data) {
                    $('#nameCity').html(data.city);
                    $('#temperature').html(Math.round(data.celsius - 273) + "&#176;")
                    $('#weather').html("Weather " + data.weather);
                    $('#timezone').html(data.time);
